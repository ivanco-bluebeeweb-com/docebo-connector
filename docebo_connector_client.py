...
class DoceboClient:
    ...
    async def list_courses(self, limit: int = 20) -> list[dict[str, Any]]:
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            resp = await client.get(f"{self.base_url}/courses", headers=self.headers, params={"limit": limit})
            if resp.status_code == 200:
                data = resp.json()
                if isinstance(data, list): return data
                for k in ["data", "courses", "items", "results"]:
    ...
    async def get_course(self, course_id: str) -> dict[str, Any]:
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            resp = await client.get(f"{self.base_url}/courses/{course_id}", headers=self.headers)
            if resp.status_code == 200:
                return resp.json()
