"""Connection lifecycle for Docebo Connector."""
...
@chat.function(
    "connect_docebo",
    "Connect Docebo account via credentials.",
    action_type="write",
    chain_callable=True,
    event="docebo-connector.connect_docebo",
    effects=["create:connection"],
    data_model=ConnectParams
)
async def connect_docebo(params: ConnectParams, ctx) -> ActionResult[ConnectionRecord]:
    """Connect Docebo Connector."""
    ...
    record = {
        "id": cid,
        "label": params.label or "Docebo Account",
...
@chat.function(
    "list_connections",
    "List connected Docebo accounts.",
    action_type="read",
    chain_callable=True,
    data_model=NoParams
)
...
@chat.function(
    "disconnect_docebo",
    "Disconnect Docebo account.",
    action_type="write",
    chain_callable=True,
    event="docebo-connector.disconnect_docebo",
    effects=["delete:connection"],
    data_model=ConnectionIdParams
)
async def disconnect_docebo(params: ConnectionIdParams, ctx) -> ActionResult[DeleteResult]:
    conns = await _load_connections(ctx)
