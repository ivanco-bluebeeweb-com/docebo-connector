"""Resource handlers for Docebo Connector."""
...
@chat.function("list_courses", "List courses in Docebo.", action_type="read", chain_callable=True, event="docebo-connector.list_courses", effects=["read:courses"], data_model=CourseList)
...
@chat.function("get_course", "Get details of one Course in Docebo.", action_type="read", chain_callable=True, event="docebo-connector.get_course", effects=["read:course"], data_model=CourseRecord)
...
@chat.function("audit_course_health", "Audit health of Docebo courses and connectivity.", action_type="read", chain_callable=True, event="docebo-connector.audit_course_health", effects=["read:audit"], data_model=AuditHealthReport)
async def audit_course_health(ctx, params: ConnectionIdParams) -> ActionResult:
    ...
    try:
        ...
        return ActionResult.success({
            ...
            "summary": f"Docebo healthy. Sampled {len(items)} courses."
        }, summary=f"Docebo health check passed with {len(items)} courses.")
    except Exception as e:
        return ActionResult.error(f"Error auditing Docebo health: {e}")
